...
CMDLINE_OPTIONS = (
    ...
    ("max-call-depth", int, lambda: -2),
    ("max-method-definition", int, lambda: -1),
    ("allow-repetitive-ngram", None, lambda: None),
    ("no-branch-ngram", None, lambda: None),
    ("exclude", [str], list),
    ("entry", [str], list),
    ("include-ctors", None, lambda: None),
#    ("bag-comparison", None, lambda: None),
)
...
class OptionHolder:
    ...
    def _format_i(self, buf, typ, rn, value):
        if typ is int:
            buf.append("# --%s=%d" % (rn, value))
        elif typ is str:
            buf.append("# --%s=%s" % (rn, value))
        elif typ is None:
            if value:
                buf.append("# --%s" % rn)
        else:
...
def read_clone_file_iter(clonefile):
    ...
    for tag, value in read_clone_iter(readline_iter(clonefile)):
        yield tag, value
