...
class TestWithSampleCode(unittest.TestCase):
    def testGenNgram(self):
        text = subprocess.check_output(["python", J(PROG_DIR, "gen_ngram.py"), "-n", "6", "-a", DATA_DIR, "--allow-repetitive-ngram"]).decode('utf-8')
    ...
    def testDoDetectionWithPipe(self):
        p1 = ' '.join(["python", J(PROG_DIR, "gen_ngram.py"), "-n", "6", "-a", DATA_DIR, "--allow-repetitive-ngram"])
    ...
    def testGenNgramWoAllowRepetition(self):
        text = subprocess.check_output(["python", J(PROG_DIR, "gen_ngram.py"), "-n", "6", "-a", DATA_DIR]).decode('utf-8')
        text_blocks = sorted(map(tuple, split_by_empty_line(text.split('\n'))))
        ref_text = read_text(J(REF_DATA_DIR, "ngram-wo-allowrepetitivengram.txt"))
        ref_text_blocks = sorted(map(tuple, split_by_empty_line(ref_text.split('\n'))))
        self.assertSequenceEqual(text_blocks, ref_text_blocks)
